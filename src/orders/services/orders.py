...
class OrderService(OrderServiceInterface):
    ...
    async def create_order(self, user_id: int) -> Order:
        ...
        if purchased_movie_ids & cart_movie_ids:
            logger.warning(f"Some movies are already purchased for user_id: {user_id}")
        ...
        logger.info(f"Order created successfully for user_id: {user_id}, order_id: {order.id}")
        return order
    ...
    async def get_order(self, user_id: int, order_id: int) -> Order:
        logger.info(f"Fetching order with order_id: {order_id} for user_id: {user_id}")
        order = await self._order_repository.get_order_by_id(order_id)
        if not order or order.user_id != user_id:
            logger.warning(
                f"Order not found or does not belong to user for order_id:"
                f" {order_id}, user_id: {user_id}")
    ...
    async def cancel_pending_order(self, user_id: int, order_id: int) -> None:
        logger.info(f"Attempting to cancel order with order_id: {order_id} for user_id: {user_id}")
        order = await self._order_repository.get_order_by_id(order_id)
        if not order or order.user_id != user_id:
            logger.warning(
                f"Order not found or does not belong to user for order_id:"
        ...
        if order.status != StatusEnum.PENDING:
            logger.warning(f"Order with order_id: {order_id} cannot be cancelled "
                           f"(status is not PENDING)")
            raise OrderUpdateError(
    ...
    async def confirm_order(self, user_id: int, order_id: int) -> Order:
        logger.info(f"Attempting to confirm order with order_id: {order_id} for user_id: {user_id}")
        order = await self._order_repository.get_order_by_id(order_id)
        if not order or order.user_id != user_id:
            logger.warning(
                f"Order not found or does not belong to user for order_id:"
        ...
        if order.status != StatusEnum.PENDING:
            logger.warning(f"Order with order_id: {order_id} cannot be confirmed "
                           f"(status is not PENDING)")
        ...
        logger.info(f"Order with order_id: {order_id} confirmed successfully, status updated to PAID")
    ...
    async def get_all_orders(
        ...
        logger.info(f"Fetching all orders with filters: user_id={user_id}, "
                    f"date_from={date_from}, date_to={date_to}, status={status}, "
